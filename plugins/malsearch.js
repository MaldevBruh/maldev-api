...
export default async function plugin(fastify, opts) {
  fastify.get('/api/malsearch', {
    schema: {
      description: 'Search anime info with MAL',
      tags: ['Search'],
      summary: 'Return a MAL Info',
      querystring: {
        query: { type: 'string', description: 'The query for search an anime' },
      },
      response: {
        200: {
          type: 'object',
    ...
    const mal = await malSearchFunction(query);
...
async function malSearchFunction(query) {
  if(!query) {
    return {
      status: false,
